...
const yamlKeys = {
  argsString: 'pandoc-options',
  format: 'pandoc-format',
  opath: 'pandoc-filepath'
}
...
module.exports = {
  ...
  activate () {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace',{
      'pandoc-convert:compile': () => this.convertCommand() })
    )
  },
  ...
  convertCommand () {
    ...
    else format = config.get('pandoc-convert.pandocDefaultFormat')
    if (!opath) {
    ...
    const pandocPath = config.get('pandoc-convert.pandocBinary')
